/*
Parte 2. Alternar el LED verde usando el retardo de SysTick. 
Este programa debe hacer uso de SysTick para generar un segundo de retraso para alternar el LED verde. 
El reloj del sistema funciona a 41,94 MHz.
SysTick está configurado para realizar una cuenta regresiva desde 41939 para dar un retraso de 1 ms. 
Por cada 1000 retrasos (1 ms * 1000 = 1 segundo), cambie el LED verde una vez. El LED verde está conectado a PTB19.
*/
...
void delayMs(int n){
	...
	for( i = 0; i < n; i++){ // count n 1 ms delays
